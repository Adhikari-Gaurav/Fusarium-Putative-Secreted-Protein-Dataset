...
Each pipeline stage lives in its own `scripts/<stage>/` folder alongside a README documenting that stage in detail. This root README stays high-level: an overview of the project plus a short summary and link for each stage.
...
Each genome assembly is run through BUSCO to assess completeness and pull out single-copy orthologous genes shared across all the assemblies. The scripts here summarize the BUSCO scores, extract the shared single-copy orthologs, align and trim them, patch in any missing samples, concatenate everything into a supermatrix, and build a phylogenetic tree from it.

Scripts and full step-by-step documentation: [`scripts/busco/`](scripts/busco/README.md).
